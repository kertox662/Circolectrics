...
void drawLayerManager() {
  ...
  if (layers.size() + numElements > maxAmount) {
    for (int i = 0; i < layers.size(); i++) {
      
        
      for(int j = 0; j < layers.get(i).texts.size() && i >= 92; j++){
        float y = height - 96  - i * 20 + managerScrolled;
        Text t = layers.get(i).texts.get(j);
        text(t.text, width-115, y);
        i++;
      }
      ...
      for(int j = 0; j < layers.get(i).components.size(); j++){
        ...
        text(c.name, width-115, i);
        i++;
      }
      
      if(i < 92) break;
      
      for(int j = 0; j < layers.get(i).tracks.size(); j++){
        float y = height - 96  - i * 20 + managerScrolled;
        LineSegment ls = layers.get(i).tracks.get(j);
        text("Line Segment", width-115, i);
        i++;
      }  
    ...
    for (int i = height - 96; i >= 92 && x < layers.size(); i-=20) {
      for(int j = 0; j < layers.get(x).texts.size() && i >= 92; j++){
        Text t = layers.get(x).texts.get(j);
        text(t.text, width-115, i);
        i-=20;
      }
      
      if(i < 92) break;
      
      for(int j = 0; j < layers.get(x).components.size(); j++){
        Component c = layers.get(x).components.get(j);
        text(c.name, width-115, i);
        i-=20;
      }
      ...
      for(int j = 0; j < layers.get(x).tracks.size(); j++){
        LineSegment ls = layers.get(x).tracks.get(j);
        text("Line Segment", width-115, i);
        i-=20;
      }
